...
namespace JBooth.VertexPainterPro
{
   ...
   public class VertexInstanceStream : MonoBehaviour
   {
      ...
      List<Vector4> cachedUV0;
      public Vector4 GetSafeUV0(int index)
      {
         if (_uv0 != null && index < _uv0.Count)
         {
            return _uv0[index];
         }
         if (cachedUV0 == null || cachedUV0.Count == 0)
         {
            MeshFilter mf = GetComponent<MeshFilter>();
            if (mf == null || mf.sharedMesh == null)
            {
            ...
            cachedUV0 = new List<Vector4>(mf.sharedMesh.vertexCount);
            mf.sharedMesh.GetUVs(0, cachedUV0);
         }
         if (index < cachedUV0.Count)
         {
            return cachedUV0[index];
         }
         return Vector4.zero;
      }
      ...
      public Vector3 GetSafeNormal(int index)
      {
         ...
         if (cachedNormals == null || cachedNormals.Length == 0)
         {
            MeshFilter mf = GetComponent<MeshFilter>();
            if (mf == null || mf.sharedMesh == null)
            {
      ...
      public Vector4 GetSafeTangent(int index)
      {
         ...
         if (cachedTangents == null || cachedTangents.Length == 0)
         {
            MeshFilter mf = GetComponent<MeshFilter>();
            if (mf == null || mf.sharedMesh == null)
            {
