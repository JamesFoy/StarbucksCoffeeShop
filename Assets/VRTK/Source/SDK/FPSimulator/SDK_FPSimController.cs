...
namespace VRTK
{
    ...
    /// </summary>
    [SDK_Description(typeof(SDK_FPSimSystem))]
    public class SDK_FPSimController : SDK_BaseController
    {
        ...
        public override GameObject GetControllerModel(ControllerHand hand)
        {
            ...
            GameObject simPlayer = SDK_FPInputSimulator.FindInScene();
            if (simPlayer != null)
            {
                switch (hand)
                {
                    case ControllerHand.Left:
                        model = simPlayer.transform.FindChildRecursive(string.Format("{0}/Hand", LEFT_HAND_CONTROLLER_NAME)).gameObject;
                        break;
                    case ControllerHand.Right:
                        model = simPlayer.transform.FindChildRecursive(string.Format("{0}/Hand", RIGHT_HAND_CONTROLLER_NAME)).gameObject;
                        break;
                }
            }
            return model;
        ...
        protected virtual void SetupPlayer()
        {
            if (rightController == null || leftController == null)
            {
                ...
                if (simPlayer != null)
                {
                    rightController = (rightController == null ? simPlayer.transform.FindChildRecursive(RIGHT_HAND_CONTROLLER_NAME).GetComponent<SDK_FPControllerSim>() : rightController);
                    leftController = (leftController == null ? simPlayer.transform.FindChildRecursive(LEFT_HAND_CONTROLLER_NAME).GetComponent<SDK_FPControllerSim>() : leftController);
                }
            }
        }
        ...
        protected virtual GameObject GetActualController(ControllerHand hand)
        {
            ...
            if (simPlayer != null)
            {
                ...
                {
                    case ControllerHand.Right:
                        controller = simPlayer.transform.FindChildRecursive(RIGHT_HAND_CONTROLLER_NAME).gameObject;
                        break;
                    case ControllerHand.Left:
                        controller = simPlayer.transform.FindChildRecursive(LEFT_HAND_CONTROLLER_NAME).gameObject;
                        break;
                    default:
                        break;
                }
