...
namespace VRTK
{
    ...
    /// </remarks>
    public class SDK_FPInputSimulator : MonoBehaviour
    {
        /// <summary>
        ...
        [Tooltip("Uses the mouse wheel to move the hands forward and backward when in mouse-look mode, " +
                 "and moves the hand along the other axis when in hand mode.")]
        public bool moveHandsWithMouseWheel = true;
        ...
        protected SDK_FPControllerSim rightController;
        protected SDK_FPControllerSim leftController;
        ...
        Vector3 leftHandInitialPosition;
        Vector3 rightHandInitialPosition;
        ...
        public static GameObject FindInScene()
        {
            ...
            {
                cachedCameraRig = VRTK_SharedMethods.FindEvenInactiveGameObject<SDK_FPInputSimulator>(null, true);
                if (!cachedCameraRig)
                {
                    VRTK_Logger.Error(VRTK_Logger.GetCommonMessage(VRTK_Logger.CommonMessageKeys.REQUIRED_COMPONENT_MISSING_FROM_SCENE, "[VRSimulator_CameraRig]", "SDK_FPInputSimulator", ". check that the `VRTK/Prefabs/CameraRigs/[VRSimulator_CameraRig]` prefab been added to the scene."));
                }
            }
        ...
        protected virtual void OnEnable()
        {
            ...
            rightHand = transform.FindChildRecursive("RightHand");
            rightHand.gameObject.SetActive(false);
            rightHandInitialPosition = rightHand.transform.localPosition;
            leftHand = transform.FindChildRecursive("LeftHand");
            ...
            neck = transform.FindChildRecursive("Neck");
            SetHandColor(leftHand, leftHandColor);
            SetHandColor(rightHand, rightHandColor);
            rightController = rightHand.GetComponent<SDK_FPControllerSim>();
            leftController = leftHand.GetComponent<SDK_FPControllerSim>();
            rightController.selected = true;
            leftController.selected = false;
            destroyed = false;

            SDK_FPSimController controllerSDK = VRTK_SDK_Bridge.GetControllerSDK() as SDK_FPSimController;
        ...
        protected virtual void Update()
        {
            ...
            else
            {
                ...
                if (moveHandsWithMouseWheel)
                {
                    var wheel = Input.GetAxis("Mouse ScrollWheel") * 100f;
                    HandleMouseInputForHands(new Vector3(0, wheel));
                }
            }
        ...
        protected virtual void UpdateHands()
        {
            ...
            {
                HandleMouseInputForHands(mouseDiff);
            }
        }

        void HandleMouseInputForHands(Vector3 mouseDiff)
        {
        ...
        protected virtual void SetHand()
        {
            ...
            {
                rightHand.transform.localPosition = rightHandInitialPosition;
                rightHand.transform.localRotation = Quaternion.identity;
                leftHand.transform.localPosition = leftHandInitialPosition;
        ...
        {
            if (hintText == null || !hintText.IsActive())
            {
              return;
            }
            ...
            hints.Clear();
            // WASD Movement
            hints.Append("Toggle Control Hints: ").AppendKey(toggleControlHints).Append("\n\n");
            hints.Append("Toggle Mouse Lock: ").AppendKey(toggleMouseLock).Append("\n");
            hints.Append("Move Player/Playspace: <b>").Append(moveForward).Append(moveLeft).Append(moveBackward).Append(moveRight).Append("</b>\n");
            hints.Append("Sprint Modifier: (").AppendKey(sprint).Append(")\n\n");
            ...
            if (isHand)
            {
                ...
                if (Input.GetKey(rotationPosition))
                {
                    hints.Append("Mouse: <b>Controller Rotation ").AppendOptionalKey(mouseMovementKey).Append("</b>\n");
                }
                else
                {
                    hints.Append("Mouse: <b>Controller Position ").AppendOptionalKey(mouseMovementKey).Append("</b>\n");
                }
                hints.Append("Modes: HMD (").AppendKey(handsOnOff).Append("), Rotation (").AppendKey(rotationPosition).Append(")\n");
                hints.Append("Controller Hand: ").Append(currentHand.name.Replace("Hand", "")).Append(" (").AppendKey(changeHands).Append(")\n");

                string axis = Input.GetKey(changeAxis) ? "X/Y" : "X/Z";
                hints.Append("Axis: ").Append(axis).Append(" (").AppendKey(changeAxis).Append(")\n");
                ...
                hints.Append("\nButton Press Mode Modifiers: Touch (").AppendKey(touchModifier).Append("), Hair Touch (").AppendKey(hairTouchModifier).Append(")\n");

                hints.Append("Trigger ").Append(pressMode).Append(": ").AppendKey(triggerAlias).Append("\n");
                hints.Append("Grip ").Append(pressMode).Append(": ").AppendKey(gripAlias).Append("\n");
                if (!Input.GetKey(hairTouchModifier))
                {
                    hints.Append("Touchpad ").Append(pressMode).Append(": ").AppendKey(touchpadAlias).Append("\n");
                    hints.Append("Button One ").Append(pressMode).Append(": ").AppendKey(buttonOneAlias).Append("\n");
                    hints.Append("Button Two ").Append(pressMode).Append(": ").AppendKey(buttonTwoAlias).Append("\n");
                    hints.Append("Start Menu ").Append(pressMode).Append(": ").AppendKey(startMenuAlias).Append("\n");
                }
            }
            else
            {
                // HMD Input
                hints.Append("Mouse: <b>HMD Rotation").AppendOptionalKey(mouseMovementKey).Append("</b>\n");
                hints.Append("Modes: Controller (").AppendKey(handsOnOff).Append(")\n");
                hints.Append("Distance Pickup Modifier: (").AppendKey(distancePickupModifier).Append(")\n");
                hints.Append("Distance Pickup Left Hand: (").AppendKey(distancePickupLeft).Append(")\n");
                hints.Append("Distance Pickup Right Hand: (").AppendKey(distancePickupRight).Append(")\n");
            }
